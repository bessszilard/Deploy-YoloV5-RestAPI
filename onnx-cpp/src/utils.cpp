...
void utils::writeToFile(std::string const &fileName, std::string const &data)
{
...
bool utils::getResponseString(std::vector<Detection> &p_detections, std::string &p_output, const std::vector<std::string> &p_labels)
{
    if (p_detections.size() == 0)
    {
        std::cout << "Invalid prediction list" << std::endl;
        return false;
    }
    p_output = "[";
    for (int i = 0; i < p_detections.size(); i++)
    {
        const auto &detection = p_detections[i];
        p_output += "{\"xmin\":" + std::to_string(detection.box.x) + ",\"ymin\":" + std::to_string(detection.box.y) +
                    ",\"xmax\":" + std::to_string(detection.box.x + detection.box.width) +
                    ",\"ymax\":" + std::to_string(detection.box.y + detection.box.height) +
                    ",\"confidence\":" + std::to_string(detection.conf) +
                    ",\"class\":" + std::to_string(detection.classId) +
                    ",\"name\":\"" + p_labels[detection.classId] + "\" }";
        if (i < p_detections.size() - 1)
            p_output += ",";
    }
    p_output += "]";
    return true;
